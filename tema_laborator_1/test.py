...
def test_3(sort_method):
	test_input = [5]
	print('N = {}, Max = {},'.format(len(test_input), max(test_input)), test_input)
...
def test_9(sort_method):
	test_input = list(random.randrange(1, 100000001) for i in range(1, 1000001))
	print('N = {}, Max = {}'.format(len(test_input), max(test_input)))
...
def test_all(sort_methods):
	...
	for index, test in enumerate(tests):
		...
		for key, method in sort_methods.items():
			if test == test_9 and key in ['bubble sort', 'count sort']:
				print('{} it takes too much time to sort'.format(key))
				continue
